...
const Day08 = () => {
    ...
    const fetchUsers = async () => {
        ...
        try {
            const usersData = await fetch('https://randomuser.me/api/?results=9');
                ...
                {loading && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {placeholder.map((_, index) => (
                            <div
                                key={index}
                                className="border border-slate-500 px-4 py-2 rounded-md flex gap-4 items-center bg-slate-800 shadow-lg w-72 animate-pulse"
                            >
